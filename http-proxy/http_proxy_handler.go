...
func Handle(buff []byte) []byte {
	...
	networkAddress := net.JoinHostPort(Hostname, port)
	conn, error := net.Dial("tcp", networkAddress)
	...
	forwardRequest(contextPath, HttpRequest, conn)
